...
class compareOrder : public std::binary_function<double,double,bool>
{ public:
        ...
        if (newObj->side == 1 && (sell.empty() || (relBegin->second->price) > newObj->price)) {
            ...
            if (!buy.empty()) {
                ...
                if (x != buy.end()) {
                    ...
                    int i = 1;
                    x++;
                    while (x->second->price == newObj->price && x != buy.end()) {
                        i++;
                        x++;
                    }
                    newObj->priority = ++i;
...
int main() {
    ...
    if (FILE* filePointer = fopen("orders.csv", "r")) {
        ...
        // <<"begin "<<(itr)->first.first<<" "<<itr->second->orderId<<"\n";
        // itr=allInstruments[0]->buy.end();
        // file<<"begin +1 "<<(itr)->first.first<<" "<<itr->second->orderId<<"\n";
        // allInstruments[0]->buy.emplace(std::make_pair(55.00,1),itr->second);
        // // file<<"npppppppp" <<allInstruments[0]->buy[std::make_pair(55.00,1)]->orderId<<"\n"
